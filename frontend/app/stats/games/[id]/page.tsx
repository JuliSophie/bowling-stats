...
interface FrameData {
  throw1: string | number;
  throw2: string | number;
  throw3?: string | number;
        ...
        <div className="rounded-[1.3rem] border border-lane-200 bg-white/90 p-5">
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {game.scores.map((score) => {
              ...
              return (
                <div key={score.player_name} className={`rounded-lg px-4 py-2 text-sm font-medium ${score.total_score === maxScore ? 'bg-lane-800 text-white' : 'bg-lane-100 text-lane-700'}`}>
                  {score.player_name}: {score.total_score} Punkte
                </div>
              );
            })}
          </div>
          ...
          {excitement && (
            <div className="mt-4 grid gap-3 rounded-xl border border-orange-200 bg-orange-50/70 p-4 sm:grid-cols-4">
              <div>
